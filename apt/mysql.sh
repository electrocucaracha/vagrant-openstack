...
sed -i "s/myisam-recover/#myisam-recover/g" /etc/mysql/my.cnf
...
mysql -uroot -p${ROOT_DBPASS} -e "CREATE DATABASE if not exists manila;"
...
mysql -uroot -p${ROOT_DBPASS} -e "CREATE DATABASE if not exists aodh;"
...
mysql -uroot -p${ROOT_DBPASS} -e "GRANT ALL PRIVILEGES ON manila.* TO 'manila'@'${SHARED_FILE_STORAGE_CONTROLLER_HOSTNAME}' IDENTIFIED BY '${MANILA_DBPASS}';"
mysql -uroot -p${ROOT_DBPASS} -e "GRANT ALL PRIVILEGES ON heat.* TO 'heat'@'${ORCHESTRATION_HOSTNAME}' IDENTIFIED BY '${HEAT_DBPASS}';"
mysql -uroot -p${ROOT_DBPASS} -e "GRANT ALL PRIVILEGES ON aodh.* TO 'aodh'@'${TELEMETRY_CONTROLLER_HOSTNAME}' IDENTIFIED BY '${AODH_DBPASS}';"
